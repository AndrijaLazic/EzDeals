...
export class AuthController {
	...
	public async createUser(
		...
		const newUser: IUserDocument = await authService.addNewUser(
			request.body as ISignUpPayload
		);
		if (!newUser) {
	...
	public async safe(request: Request, response: Response): Promise<void> {
		response.status(HTTP_STATUS.OK).json({ message: "Succesfull safe" });
	}
}
